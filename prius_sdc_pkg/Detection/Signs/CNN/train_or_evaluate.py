import os
import pathlib
from prius_sdc_pkg.config import config
import numpy as np
...
def EvaluateModelOnImage(model_path,image_path = "",image_label = None):
    ...
    # load dataset
    if image_path == "":
        def_test_images_path = "prius_sdc_pkg/prius_sdc_pkg/data/test_images"
        # Number of Classes
        global NUM_CATEGORIES
...
def main():
    if config.Training_CNN:
        train_SignsModel(os.path.abspath("prius_sdc_pkg/prius_sdc_pkg/data/"))
    if Evaluate_Model:
        EvaluateModelOnImage(os.path.abspath("prius_sdc_pkg/prius_sdc_pkg/data/saved_model_5_Sign.h5"))
